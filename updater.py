...
Installed builds only (no portable mode):
...
Flow:
  ...
  3. On "Download + Install", the installer is downloaded to a temp dir:
     Windows runs the Setup installer (silent) and exits;
     Linux installs the .deb via pkexec + apt, then relaunches.
